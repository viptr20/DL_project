...
from src.models import DualAgentModel
from src.dataset import Coil100Dataset


MODEL_PATH = "./checkpoints/dual_resnet18.pt"
AGENT_NAME = "agent_a"
...
def get_agent_embedding(model, x, agent_name="agent_a"):
    if agent_name == "agent_a":
        h, _ = model.agent_a(x)
    elif agent_name == "agent_b":
        h, _ = model.agent_b(x)
    else:
        raise ValueError(f"Невалиден agent_name: {agent_name}")
...
def load_model_and_data():
    ...
    model = DualAgentModel(cfg.embedding_dim).to(device)
    ...
    with torch.no_grad():
        for sample in dataset:
            ...
            h = get_agent_embedding(model, x, agent_name=AGENT_NAME)
            feats.append(h.cpu())
...
def main():
    st.set_page_config(page_title="Dual-agent групиране", layout="wide")
    st.title("Детски-вдъхновено двуагентно самообучение за групиране на визуални обекти")
    st.write("Качи изображение и виж най-близките подобни обекти от COIL-100 чрез dual-agent модел.")
    st.caption(f"Използван embedding agent: {AGENT_NAME}")
